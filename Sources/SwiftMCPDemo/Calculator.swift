...
class Calculator {
    ...
    /// - Parameter name: Name of the person to greet (defaults to "World")
    ...
    /// - Parameter exponent: The exponent (defaults to 2)
    /// - Returns: The base raised to the power of the exponent
    @MCPFunction
    func power(base: Double, exponent: Int = 2) -> Double {
        return pow(base, Double(exponent))
    }
    
    /// Formats a number with a specified precision
    /// - Parameter value: The number to format
    /// - Parameter precision: The number of decimal places (defaults to 2)
    /// - Parameter includeThousandsSeparator: Whether to include thousands separators (defaults to true)
    /// - Returns: The formatted number as a string
    @MCPFunction
    func formatNumber(value: Double, precision: Int = 2, includeThousandsSeparator: Bool = true) -> String {
        let formatter = NumberFormatter()
        formatter.numberStyle = .decimal
        formatter.maximumFractionDigits = precision
        formatter.usesGroupingSeparator = includeThousandsSeparator
