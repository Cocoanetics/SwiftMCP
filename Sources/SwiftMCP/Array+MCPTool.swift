	...
	public func convertedToTools() -> [MCPTool]
	{
		return self.map { meta in
			...
			for parameter in meta.parameters {
				properties[parameter.name] = MCPTool.JSONSchema.Property(type: parameter.type.JSONSchemaType)
				required.append(parameter.name)
			}
			
			let schema = MCPTool.JSONSchema(type: "object", properties: properties.isEmpty ? nil : properties, required: required.isEmpty ? nil : required)
