...
final class HTTPHandler: NSObject, ChannelInboundHandler, Identifiable, @unchecked Sendable, URLSessionTaskDelegate {
    ...
    /// Upload requests use the server's `maxUploadSize`; everything else uses `maxMessageSize`.
        ...
        do {
            ...
            await transport.sessionManager.session(id: sessionID).work { session in
                ...
                if await session.hasActiveConnection {
                    ...
                    if transport.server is MCPFileUploadHandling {
                        let paths = await transport.uploadStore.filePaths(for: sessionID)
    ...
    private func handleUpload(channel: Channel, head: HTTPRequestHead, body: ByteBuffer?) async {
        // Check server conformance
        guard let uploadHandler = transport.server as? MCPFileUploadHandling else {
        ...
        // Check size
        guard data.count <= uploadHandler.maxUploadSize else {
            let buffer = channel.allocator.buffer(string: "Upload exceeds maximum size of \(uploadHandler.maxUploadSize) bytes.")
            await sendResponseAsync(channel: channel, status: .payloadTooLarge, body: buffer)
            return
        }
        ...
        do {
            ...
            if let ct = upload.contentType {
                responseDict["contentType"] = .string(ct)
            }
            if let fn = upload.filename {
                responseDict["filename"] = .string(fn)
            }
            ...
            await sendResponseAsync(channel: channel, status: .created, headers: headers, body: buffer)

            logger.info("File upload stored: \(upload.uri) (\(upload.size) bytes)")
        } catch {
            logger.error("File upload failed: \(error)")
            let buffer = channel.allocator.buffer(string: "Upload failed: \(error.localizedDescription)")
