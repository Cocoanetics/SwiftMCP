...
/**
	 ...
	 The requirement is not `nonisolated` so that actor-backed conformers can
	 keep this property actor-isolated. Concrete class hosts emitted by
	 `@MCPServer` still expose it as `nonisolated`, so existing class-based
	 call sites read it synchronously without an `await`.
	 */
    var mcpToolMetadata: [MCPToolMetadata] { get async }
