...
extension MCPToolProviding {
    /// Registers extension-defined tools for this server type.
    ///
    /// Called by code generated by the SwiftMCPAggregator build plugin from
    /// inside a `SwiftMCPBootstrap_<Module>.register()` function. Idempotent
    /// across modules — duplicates by name are silently dropped.
    public static func __mcpRegisterExtensionTools(_ tools: [MCPExtensionToolEntry]) {
        MCPExtensionRegistry.register(Self.self, tools: tools)
    }
}
