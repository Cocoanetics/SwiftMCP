...
/**
    ...
    /**
         Retrieves metadata for a function annotated with `@MCPTool`.

         - Parameter toolName: The name of the tool
         - Returns: The metadata for the tool if available
         */
    func mcpToolMetadata(for toolName: String) -> MCPToolMetadata? {
        guard let provider = self as? MCPToolProviding else {
            return nil
        }

        return provider.mcpToolMetadata.first { $0.name == toolName }
    }

    /// Retrieves metadata for a prompt function by name
    func mcpPromptMetadata(for name: String) -> MCPPromptMetadata? {
        guard let provider = self as? MCPPromptProviding else {
            return nil
        }

        return provider.mcpPromptMetadata.first { $0.name == name }
    }
}
