...
"""
        ...
        // Create a direct call wrapper (like MCPTool) for OpenAPI compatibility
        var directCallWrapper = """

        /// Autogenerated direct wrapper for \(functionName) that returns the function result directly (for OpenAPI)
        func __mcpDirectCall_\(functionName)(_ enrichedArguments: [String: Sendable]) async throws -> (Encodable & Sendable) {
        """

        for detail in wrapperParamDetails {
            directCallWrapper += """
                let \(detail.name): \(detail.type) = try enrichedArguments.extractValue(named: "\(detail.name)", as: \(detail.type).self)
            """
        }

        if returnTypeString == "Void" {
            directCallWrapper += """
                \(commonMetadata.isThrowing ? "try " : "")\(commonMetadata.isAsync ? "await " : "")\(functionName)(\(callParameterList))
                return ""  // return empty string for Void functions
            }
            """
        } else {
            directCallWrapper += """
                return \(commonMetadata.isThrowing ? "try " : "")\(commonMetadata.isAsync ? "await " : "")\(functionName)(\(callParameterList))
            }
            """
        }
        ...
        return [
            ...
            DeclSyntax(stringLiteral: directCallWrapper)
        ]
    }
}
