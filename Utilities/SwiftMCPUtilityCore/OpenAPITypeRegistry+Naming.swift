...
extension OpenAPITypeRegistry {
    ...
    /// compared with descriptions stripped: the same fields documented two
    /// ways are still one type. (`required` is a set, so the order it was
    /// written in never enters into it.)
    ///
