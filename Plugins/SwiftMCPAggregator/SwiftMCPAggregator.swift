...
//  SwiftMCPAggregatorTool to emit aggregated Client extensions for every
//  `@MCPExtension` in the target.
//
...
struct SwiftMCPAggregator: BuildToolPlugin {
    func createBuildCommands(context: PluginContext, target: Target) async throws -> [Command] {
        ...
        let outputFile = outputDir.appending(path: "SwiftMCPClientExtensions_\(sanitize(target.name)).swift")
        ...
        return [
            .buildCommand(
                displayName: "Aggregating MCP Client extensions for \(target.name)",
                executable: tool.url,
                arguments: arguments,
                inputFiles: inputs,
